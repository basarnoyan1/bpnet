"""
PyTorch-compatible genomics data extractors to replace genomelake functionality
"""
...
from . import backend # Added import for backend
...
def one_hot_encode_sequence(seq, encoded):
    ...
    for i, base in enumerate(seq):
        ...
        if col_idx >= 0:
            ...
            # For N or unknown bases, use equal probability
            encoded[i, :] = 0.25
...
class BaseExtractor(object):
    """Base class for extractors compatible with genomelake.extractors.BaseExtractor"""
    dtype = np.float32
...
class FastaExtractor(BaseExtractor):
    ...
    def __init__(self, datafile, use_strand=False, **kwargs):
        ...
        NOTE: The extractor is not thread-safe.
        If you wish to use it with multiprocessing,
        create a new extractor object in each process.
        
        Args:
          datafile (str): path to the fasta file
...
class ArrayExtractor(BaseExtractor):
    ...
    def __init__(self, datafile, in_memory=False, **kwargs):
        ...
        self._data = self._load_directory(datafile, in_memory=in_memory)
    ...
    def _load_directory(self, base_dir, in_memory=False):
        """Load genomelake data directory"""
        # Uses the backend.load_directory function
        return backend.load_directory(base_dir, in_memory=in_memory)
