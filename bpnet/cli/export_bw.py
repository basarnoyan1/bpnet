...
from bpnet.utils import add_file_logging, create_tf_session
...
@arg("output_prefix", help="if True, the output directory will be overwritten")
@arg(
    "--regions",
...
def bpnet_export_bw(
    ...
    bp.export_bw(
        ...
        chromosomes=None,
    )  # infer chromosomes from the fasta file
...
def contrib2bw(contrib_file, output_prefix):
    ...
    for contrib_score in contrib_scores:
        ...
        for task, contrib in contrib_dict.items():
            output_file = (
                output_prefix + f"{delim}.contrib.{contrib_score_name}.{task}.bw"
            )
            logger.info(f"Genrating {output_file}")
            contrib_writer = BigWigWriter(
                output_file, chrom_sizes=chrom_sizes, is_sorted=False
            )

            for idx in range(len(ranges)):
                contrib_writer.region_write(
                    region={
