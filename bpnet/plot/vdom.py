...
import numpy as np
import pandas as pd
from vdom.helpers import p, li, div, b, br, ul, img, details, summary
...
from bpnet.plot.profiles import (
    extract_signal,
    multiple_plot_stranded_profile,
    hist_position,
    bar_seqlets_per_example,
    box_counts,
)
...
def vdom_footprint(
    arr, r_height=None, text=None, fontsize=32, figsize=(3, 1), **kwargs
):
...
def template_vdom_pattern(
    ...
    return details(
        ...
        details(
            summary("Contribution scores (profile)"),
            img(src=figures_url + "/contrib_profile.png", width=840),
        ),
        details(
            summary("Contribution scores (counts)"),
...
def vdom_pattern(
    ...
    trimmed_motif = vdom_pssm(
        mr.get_pssm(metacluster + "/" + pattern, rc=False, trim_frac=trim_frac),
        letter_width=letter_width,
        height=height,
    )
    full_motif = vdom_pssm(
        mr.get_pssm(metacluster + "/" + pattern, rc=False, trim_frac=0),
        letter_width=letter_width,
        height=height,
    )
    ...
    add_plots = OrderedDict(
        [
            ...
            (
                ...
                [
                    p(
                        f"Pattern occurs in {len(pattern_idx)} / {len(total_counts)} regions"
...
def template_vdom_metacluster(
    name, n_patterns, n_seqlets, important_for, patterns, is_open=False
):
    return details(
        summary(
            b(name),
...
def vdom_metacluster(
    mr, metacluster, figdir, total_counts, dfp=None, is_open=True, **kwargs
):
    patterns = mr.pattern_names(metacluster)
    n_seqlets = sum([mr.n_seqlets(metacluster + "/" + pattern) for pattern in patterns])
    n_patterns = len(patterns)

    def render_act(task, act):
        """Render the activity vector"""
        task = task.replace("/weighted", "").replace(
            "/profile", ""
        )  # omit weighted or profile
        if act == 0:
            return ""
        elif act < 0:
            return f"-{task}"
    ...
    important_for = ",".join(
        [render_act(task, act) for task, act in zip(mr.tasks(), activities) if act != 0]
    )
    pattern_vdoms = [
        vdom_pattern(mr, metacluster, pattern, figdir, total_counts, dfp, **kwargs)
        for pattern in patterns
    ]
    return template_vdom_metacluster(
        metacluster,
        n_patterns,
        n_seqlets,
        important_for,
...
def get_signal(seqlets, d: ContribFile, tasks, resize_width=200):
    ...
    # prepare data
    ex_signal = {
        task: extract_signal(d.get_profiles()[task], valid_seqlets) for task in tasks
    }
    ...
    if d.contains_contrib_score("count"):
        ex_contrib_counts = {
            task: extract_signal(d.get_contrib("count")[task], valid_seqlets).sum(
                axis=-1
            )
            for task in tasks
        }
        ...
        ex_contrib_counts = {
            task: extract_signal(
                d.get_contrib("counts/pre-act")[task], valid_seqlets
            ).sum(axis=-1)
            for task in tasks
        }
    else:
    ...
    _, contrib, hyp_contrib, profile, ranges = d.get_all()
...
def vdm_heatmaps(
    ...
    return div(
        details(
            ...
            fig2vdom(
                heatmap_sequence(
                    ex_seq, sort_idx=sort_idx, figsize_tmpl=(10, 15), aspect="auto"
                )
            ),
            open=opened,
        ),
        details(
            ...
            fig2vdom(
                multiple_heatmap_stranded_profile(
                    ex_signal, sort_idx=sort_idx, figsize=(20, 20)
                )
            ),
            open=opened,
        ),
        details(
            ...
            fig2vdom(
                multiple_heatmap_contribution_profile(
                    ex_contrib_profile, sort_idx=sort_idx, figsize=(20, 20)
                )
            ),
            open=opened,
        ),
        details(
            ...
            fig2vdom(
                multiple_heatmap_contribution_profile(
                    ex_contrib_counts, sort_idx=sort_idx, figsize=(20, 20)
                )
            ),
            open=opened,
        ),
...
def write_heatmap_pngs(seqlets, d, tasks, output_dir, resize_width=200):
    """Write out histogram png's"""
    # get the data
    ex_signal, ex_contrib_profile, ex_contrib_counts, ex_seq, sort_idx = get_signal(
        seqlets, d, tasks, resize_width=resize_width
    )
    # get the plots
    figs = dict(
        heatmap_seq=heatmap_sequence(
            ex_seq, sort_idx=sort_idx, figsize_tmpl=(10, 15), aspect="auto"
        ),
        profile_aggregated=multiple_plot_stranded_profile(
            ex_signal, figsize_tmpl=(20 / len(ex_signal), 3)
        ),
        profile_heatmap=multiple_heatmap_stranded_profile(
            ex_signal, sort_idx=sort_idx, figsize=(20, 20)
        ),
        contrib_profile=multiple_heatmap_contribution_profile(
            ex_contrib_profile, sort_idx=sort_idx, figsize=(20, 20)
        ),
    )

    if ex_contrib_counts is not None:
        figs["contrib_counts"] = multiple_heatmap_contribution_profile(
            ex_contrib_counts, sort_idx=sort_idx, figsize=(20, 20)
        )
    # write the figures
...
def df2html(df, uuid="table", style="width:100%"):
    ...
    return s.render(uuid=uuid).replace(
        f'<table id="T_{uuid}"',
...
def df2html_old(df, style="width:100%"):
    ...
    with pd.option_context("display.max_colwidth", -1):
        table = df.to_html(
            escape=False, classes="display nowrap", float_format="%.2g", index=False
        ).replace(' class="dataframe', f' {add_tags} class="dataframe')
    return table
