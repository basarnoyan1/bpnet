...
def spaced_xticks(ax, spacing=5):
    ax.set_xticks(
        list(range(int(ax.get_xlim()[0]) + 1, int(ax.get_xlim()[1]) + 1, int(spacing)))
    )
...
def draw_hline(start, end, y, ax, col="r", linewidth=5, alpha=0.1):
    ...
    lc = LineCollection(
        [[(start, y), (end, y)]], colors=col, linewidths=linewidth, alpha=alpha
    )
...
class PlotninePalette:
    ...
    def __init__(self, mpl_cmap, limits=None):
        """
        Args:
    ...
    def __call__(self, values):
        ...
        return [mpl.colors.to_hex(x) for x in plt.get_cmap(self.mpl_cmap)(values)]
...
def plot_colormap(cmap_dict, label="", ax=None, figwidth=0.5):
    """Plot a color map
    ...
    def major_formatter(x, pos):
        return {D + i / len(keys): k for i, k in enumerate(keys)}[x]
