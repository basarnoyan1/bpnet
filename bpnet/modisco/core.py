...
from bpnet.modisco.sliding_similarities import sliding_similarity, pssm_scan
from bpnet.modisco.utils import ic_scale, trim_pssm_idx
from bpnet.modisco.utils import shorten_pattern
from bpnet.plot.tracks import (
    plot_tracks,
    filter_tracks,
    rc_tracks,
    skip_nan_tracks,
...
from bpnet.stats import (
    fdr_threshold_norm_right,
    quantile_norm,
    low_medium_high,
    get_metric,
)
...
class Pattern:
    ...
    def __init__(self, name, seq, contrib, hyp_contrib, profile=None, attrs=None):
        """Pattern container
    ...
    def to_summary_dict(self, properties=None):
        """Summarize the pattern to properties"""
        if properties is None:
            properties = ["name", "seq_info_content"]
    ...
    def from_hdf5_grp(cls, grp, name):
        ...
        if possible_names == ["fwd", "rev"]:
            ...
            if len(contrib_name) == 1:
                ...
                hyp_contrib_name = [
                    pn for pn in possible_names if "hypothetical_contribs" in pn
                ][0]
                return Pattern(
                    name,
                    seq=grp["sequence"]["fwd"][:],
                ...
                return Pattern(
                    ...
                    hyp_contrib={
                        t: grp[t][grp_1][hyp_contrib_name]["fwd"][:] for t in tasks
                    },
                )
    ...
    def append_pfm_to_database(self, meme_db, trim_frac=0.08, freqs=None):
        """write the pfm into a meme file to be used by motif search tools"""
        if freqs is None:
            freqs = [0.27, 0.23, 0.23, 0.27]
        ...
        with open(meme_db, "a") as fp:
            fp.write("MOTIF {}\n".format(motif_name))
            fp.write(
                "letter-probability matrix: alength= 4 w= {} nsites= 20 E= 0e+0\n".format(
                    pfm.shape[0]
                )
            )
            for line in pfm:
    ...
    def append_pwm_to_database(self, pwm_db, trim_frac=0.08):
        ...
        if os.path.exists(
            pwm_db
        ):  # search whether the pwm already exists in the database
    ...
    def fetch_tomtom_matches(
        ...
        tget_idx, pval_idx, eval_idx, qval_idx = (
            schema.index("Target_ID"),
            schema.index("p-value"),
            schema.index("E-value"),
            schema.index("q-value"),
        )
        r = []
        for t in dat:
            if len(t) < 4:
                break
            mtf = {"Target ID": t[tget_idx], "p-value": float(t[pval_idx]), "E-value": float(t[eval_idx]),
                   "q-value": float(t[qval_idx])}
    ...
    def resize(self, new_len, anchor="center"):
        ...
        if anchor != "center":
            raise NotImplementedError(
                "Only anchor='center' is implemented at the moment"
            )
    ...
    def shift(self, offset=0, pad_value=0):
        ...
        return padded.trim(start, start + len(self)).trim_profile(
            start, start + self.len_profile()
        )
    ...
    def plot(
        self, kind="all", rotate_y=90, letter_width=0.2, height=0.8, ylab=True, **kwargs
    ):
        ...
        if "seq" in tracks:
            tracks["seq"] = (
                self.get_seq_ic()
            )  # override the sequence with information content
    ...
    def vdom_plot(
        self, kind="seq", width=80, letter_width=0.2, letter_height=0.8, as_html=False
    ):
        """Get the html"""
        ...
        if kind == "contrib":
            ...
            arr = mean(
                list(self.contrib.values())
            )  # average the contrib scores across tasks
        elif kind == "hyp_contrib":
            # summarize across tasks
            arr = mean(
                list(self.hyp_contrib.values())
            )  # average the contrib scores across tasks
        elif kind == "seq":
            # get the IC
            arr = self.get_seq_ic()
        ...
        vdom_obj = vdom_pssm(
            arr, letter_width=letter_width, letter_height=letter_height
        )
        if as_html:
    ...
    def similarity(
        self, pattern, track="seq", metric="continousjaccard", max_score=True
    ):
        ...
        if track.startswith("profile"):
            ...
            if isinstance(pattern, list):
                pattern = [p.resize_profile(motif_len * 3 - 1) for p in pattern]
            else:
                pattern = pattern.resize_profile(motif_len * 3 - 1)
        else:
            motif_len = len(self)
            if isinstance(pattern, list):
                pattern = [p.resize(motif_len * 3 - 1) for p in pattern]
            else:
                pattern = pattern.resize(motif_len * 3 - 1)

        if isinstance(pattern, list):
            match_scores = sliding_similarity(
                self._get_track(track)[np.newaxis],
                np.concatenate([p._get_track(track)[np.newaxis] for p in pattern]),
                metric=metric,
                verbose=False,
                n_jobs=1,
            )
    ...
    def align(
        ...
        Returns:
          (float) similarity score. The higher, the more similar the two patterns are
        """
        similarities_fwd = self.similarity(
            pattern, track=track, metric=metric, max_score=False
        )
        similarities_rev = self.rc().similarity(
            pattern, track=track, metric=metric, max_score=False
        )
        ...
        if max_shift is not None and np.abs(offset) > max_shift:
            ...
            return obj.shift(offset, pad_value=pad_value).add_attr(
                "align", {"use_rc": use_rc, "offset": offset}
            )
    ...
    def scan_raw(
        ...
        if seq is not None:
            seq_scan = pssm_scan(
                self.seq, seq, n_jobs=n_jobs, pad_mode=pad_mode, verbose=verbose
            )
        if contrib is not None:
            contrib_scan = {
                task: sliding_similarity(
                    self.contrib[task],
                    contrib[task],
                    metric="continousjaccard",
                    pad_mode=pad_mode,
    ...
    def scan_profile(self, profile, pad_mode="median", n_jobs=8, verbose=True):
        ...
        _, _, _, profile_scan = self.scan_raw(
            None, None, None, profile, pad_mode, n_jobs, verbose=verbose
        )
        _, _, _, profile_scan_rc = self.rc().scan_raw(
            None, None, None, profile, pad_mode, n_jobs, verbose=verbose
        )
    ...
    def scan_seq(self, seq, pad_mode="median", n_jobs=8, verbose=True):
        ...
        seq_scan_rc = pssm_scan(
            self.rc().seq, seq, n_jobs=n_jobs, pad_mode=pad_mode, verbose=verbose
        )
    ...
    def get_instances_seq_scan(self, seq, threshold=3, n_jobs=8, verbose=True):
        ...
        positions = np.broadcast_to(
            np.arange(match.shape[1]).reshape((1, -1)), match.shape[:2]
        ).reshape((-1,))
        example_idx = np.broadcast_to(
            np.arange(match.shape[0]).reshape((-1, 1)), match.shape[:2]
        ).reshape((-1,))
    ...
    def scan_contribution(
        self, contrib, tasks, pad_mode="median", n_jobs=8, verbose=True
    ):
        """Scan the tracks using this pattern
        ...
        # fwd
        _, contrib_scan, _, _ = self.scan_raw(
            None, contrib, None, None, pad_mode, n_jobs, verbose=verbose  # hyp_contrib,
        )
        # stack by tasks
        contrib_scan_match = np.stack([contrib_scan[t][0] for t in tasks], axis=-1)
        contrib_scan_contribution = np.stack(
            [contrib_scan[t][1] for t in tasks], axis=-1
        )

        # rev
        _, contrib_scan_rc, _, _ = self.rc().scan_raw(
            None, contrib, None, None, pad_mode, n_jobs, verbose=verbose  # hyp_contrib,
        )
        # stack by tasks
        contrib_scan_match_rc = np.stack(
            [contrib_scan_rc[t][0] for t in tasks], axis=-1
        )

        return (
            np.stack([contrib_scan_match, contrib_scan_match_rc], axis=-1),
            contrib_scan_contribution,
        )  # contribution doesn't have the strand axis

    def get_task_contribution(self, kind="contrib"):
        tasks = self.tasks()
        task_contrib = np.array(
            [np.abs(self._get_track(kind)[t]).mean() for t in tasks]
        )
    ...
    def get_instances(
        ...
        # sigma = 1.4826*MAD
        if norm_df is None:
            threshold = fdr_threshold_norm_right(
                match_score, skip_percentile=skip_percentile, fdr=fdr
            )
            keep = match_score > threshold
            ...
            if verbose:
                print(
                    f"Keeping {keep.sum()}/{len(keep)} ({keep.mean():.2%}) of the instances."
        ...
        df = pd.DataFrame(
            OrderedDict(
                [
                    ...
                    (
                        "match_max_task",
                        pd.Series(match_max_task[keep]).map(
                    ...
                    (
                        "contrib_max_task",
                        pd.Series(contrib_max_task[keep]).map(
                            {i: t for i, t in enumerate(tasks)}
                        ),
                    ),
                ]
    ...
    def _trim_center_shift(self, trim_frac):
        ...
        return (
            (halve(trim_len)[0] + trim_i) - halve(orig_len)[0],
...
def patterns_to_df(patterns: List[Pattern], properties: List[str]) -> pd.DataFrame:
    ...
    return pd.DataFrame([p.to_summary_dict(properties) for p in patterns])
...
class StackedSeqletContrib:
    ...
    def __init__(
        self, seq, contrib, hyp_contrib, profile=None, name=None, dfi=None, attrs=None
    ):
    ...
    def __getitem__(self, idx):
        if isinstance(idx, np.ndarray) or isinstance(idx, pd.Series):
            ...
            return StackedSeqletContrib(
                name=self.name,
                seq=self.seq[idx],
                contrib={t: self.contrib[t][idx] for t in self.tasks()},
                hyp_contrib={t: self.hyp_contrib[t][idx] for t in self.tasks()},
                profile={t: self.profile[t][idx] for t in self.tasks()},
                dfi=dfi_subset,
                attrs=self.attrs,
            )
        else:
    ...
    def plot(self, kind="profile", **kwargs):
        ...
        """
        from bpnet.plot.heatmaps import (
    ...
    @classmethod
    def concat(cls, stacked_seqlet_list):
...
class Seqlet:
    ...
    def __getitem__(self, item):
        if item == "example":
            ...
            raise ValueError(
                "item needs to be from:" "example, start, end, pattern, rc"
            )
    ...
    def extract(self, x, rc_fn=lambda x: x[::-1, ::-1]):
        ...
        if isinstance(x, OrderedDict):
            return OrderedDict(
                [(track, self.extract(arr, rc_fn)) for track, arr in x.items()]
            )
        elif isinstance(x, dict):
            return {track: self.extract(arr, rc_fn) for track, arr in x.items()}
        elif isinstance(x, list):
            return [(track, self.extract(arr, rc_fn)) for track, arr in x]
        else:
            # Normal array
            ...
            return optional_rc(
                x[self["example"]][self["start"] : self["end"]], self["rc"]
            )
    ...
    def valid_resize(self, width, max_width):
        if width is None:
            width = self.width()
        return width // 2 < self.center() < max_width - width // 2
...
def resize_seqlets(seqlets, resize_width, seqlen):
    return [
        s.resize(resize_width) for s in seqlets if s.valid_resize(resize_width, seqlen)
    ]
