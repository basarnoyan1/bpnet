...
from bpnet.plot.vdom import vdom_footprint
...
def similarity_matrix(patterns, **kwargs):
    ...
    for i in tqdm(range(len(patterns))):
        for j in range(len(patterns)):
            m[i, j] = max(
                patterns[i].similarity(patterns[j], **kwargs),
                patterns[i].rc().similarity(patterns[j], **kwargs),
            )
    return (m + m.T) / 2


def append_logo_cluster(
    pattern_table,
    patterns,
    cluster_order,
    cluster,
    ...
    **kwargs,
    ...
    patterns_nte_dict = {
        shorten_pattern(p.name): p for p in patterns
    }  # organize as a dict
    ...
    for cluster_id in tqdm(pattern_table_nte.cluster.unique()):
        ...
        logo_contrib = [
            patterns_nte_dict[p]
            .align(major_pattern, track=align_track)
            .resize(logo_len)
            .vdom_plot("contrib", as_html=True, **kwargs)
            for p in dfg.index
        ]
        logo_seq = [
            patterns_nte_dict[p]
            .align(major_pattern, track=align_track)
            .resize(logo_len)
            .vdom_plot("seq", as_html=True, **kwargs)
            for p in dfg.index
        ]
...
def to_colors(df, cat_cmap="RdBu_r", num_cmap="Reds"):
    ...
    Args:
      num_cmap:
      df: pandas.DataFrame
      cat_cmap: Seaborn color pallete to use. Example: RdBu_r, Reds
...
def append_features(df):
    df = df.copy()
    df["metacluster"] = (
        df.pattern.str.split("_", expand=True)[0].str.replace("m", "").astype(int)
    )
...
def preproc_motif_table(df, tasks, unclust_feat=None, drop_columns=None):
    """Preprocess motif table"""
    if unclust_feat is None:
        unclust_feat = ["log n seqlets"]
    ...
    # drop some columns
    drop = (
        ["idx", "consensus", "metacluster", "n seqlets", "index"]
        + unclust_feat
        + drop_columns
    )
    ...
    row_df = df_minmax_scale(
        df[unclust_feat]
    )  # TODO - make it general in case the columns are categories

    # column
    col_df = pd.DataFrame(
        dict(
            task=pd.Series(dfx.columns, index=dfx.columns).str.split(
                " ", n=1, expand=True
            )[0],
            feature=pd.Series(dfx.columns, index=dfx.columns).str.split(
                " ", n=1, expand=True
            )[1],
        )
    )
...
def motif_table_long(df, tasks, index_cols=None):
...
def hirearchically_reorder_table(df, tasks):
    ...
    # pos std
    columns = [c for c in df.columns if c not in dfx.columns] + [
        c for c in col_idx if c in df.columns
    ]
...
def get_major_patterns(patterns, cluster):
    ...
    try:
        df = pd.DataFrame(
            {
                "n_seqlets": [p.attrs["features"]["n seqlets"] for p in patterns],
                "cluster": cluster,
            }
        )
    except:
        ...
        df = pd.DataFrame(
            {"n_seqlets": np.random.randn(len(cluster)), "cluster": cluster}
        )
    return OrderedDict(
        [(k, patterns[v]) for k, v in df.groupby("cluster").n_seqlets.idxmax().items()]
    )
    ...
    for i in order:
        ...
        if prev is None:
            ...
            if new_align_strategy == "prev":
                ...
                p_aligned = p.align(
                    prev, track=align_track, max_shift=max_shift, metric=metric
                )
            elif new_align_strategy == "best_prev":
                similarities = [
                    p.similarity(p_prev, track=align_track, metric=metric)
                    for p_prev in out
                ]
                ...
                p_aligned = p.align(
                    out[align_to], track=align_track, metric=metric, max_shift=max_shift
                )
            elif new_align_strategy == "all_prev":
                total_sim += p.similarity(out, track=align_track, metric=metric)
                p_aligned = p.align(
                    out, track=align_track, metric=metric, max_shift=max_shift
                )
            else:
                raise ValueError(
                    f"invalid new_align_strategy={new_align_strategy}. Use 'prev' or 'all_prev'"
                )
...
def align_clustered_patterns(
    ...
    major_patterns = optimal_align_patterns(
        get_major_patterns(patterns, cluster),
        cluster_order_ind,
        max_shift=max_shift,
        metric=metric,
        trials=trials,
    ...
    return [
        patterns[i]
        .align(major_patterns[cluster[i]], metric=metric, track=align_track)
        .add_attr("cluster", cluster[i])
...
def cluster_patterns(patterns, n_clusters=9, cluster_track="seq_ic"):
    ...
    pattern_table_nte_seq = create_pattern_table(
        lm_nte_seq,
        cluster_order,
        cluster,
        align_track="contrib/mean",
...
def create_pattern_table(
    ...
    profile_max_median = {
        t: np.median([p.profile[t].max() for p in patterns]) for t in tasks
    }
    ...
    def extract_info(pattern_i):
        ...
        d = {
            ...
            **dict_suffix_key(p.attrs.get("motif_odds_p", dict()), "/odds_p"),
            "cluster": p.attrs["cluster"],
            "logo_contrib": p.resize(logo_len).vdom_plot(
                "contrib", as_html=True, **seqlogo_kwargs
            ),
            "logo_seq": p.resize(logo_len).vdom_plot(
                "seq", as_html=True, **seqlogo_kwargs
            ),
        }
    ...
    l = Parallel(n_jobs=n_jobs)(
        delayed(extract_info)(i) for i in tqdm(range(len(patterns)))
    )
    df = pd.DataFrame(l)
...
def cluster_align_patterns(patterns, n_clusters=9, trials=20, max_shift=15):
    ...
    # align patterns
    patterns_clustered = align_clustered_patterns(
        patterns,
        cluster_order,
        cluster,
        align_track="seq_ic",
...
def get_df_info(patterns, tasks):
    ...
    for p in patterns:
        ...
        d_footprint = {t + "_max": p.profile[t].max() for t in tasks}
        d = {**d, **d_footprint}
        pinfo.append(d)
